import { createBrowserStorageAdapter, SAVE_KEY, RECOVERY_KEY } from './storage.js';
...
export function saveGame(state, adapter = createBrowserStorageAdapter()) { adapter.write(serializeSave(state)); return state; }
export function loadGame(adapter = createBrowserStorageAdapter()) { const raw = adapter.read(); if (!raw) return null; try { return deserializeSave(raw); } catch { adapter.clear(); return null; } }
